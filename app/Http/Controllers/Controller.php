...
class Controller extends BaseController
{
    ...
    public  function getTapChuaMuaByPhim($userId, $phim)
    {
    ...
    public function getAllTapByPhim($phim, $userId)
    {
        $listTap = DB::table($this->_PRFIX_TABLE . '_postmeta')
            ...
            ->where($this->_PRFIX_TABLE . '_postmeta.meta_value', $phim)
            ->where($this->_PRFIX_TABLE . '_posts.post_status', 'publish')
            ->pluck($this->_PRFIX_TABLE . '_postmeta.post_id')
            ->filter(function ($tap) {
                $price = $this->getPostMeta($tap, '_regular_price');
                return $price > 0;
            });

        $listTapOrder = DB::table($this->_PRFIX_TABLE . '_wc_orders')
            ->join($this->_PRFIX_TABLE . '_wc_order_product_lookup', $this->_PRFIX_TABLE . '_wc_order_product_lookup.order_id', '=', $this->_PRFIX_TABLE . '_wc_orders.id')
            ->where($this->_PRFIX_TABLE . '_wc_orders.status', 'wc-completed')
            ->where($this->_PRFIX_TABLE . '_wc_orders.customer_id', $userId)
            ->whereIn($this->_PRFIX_TABLE . '_wc_order_product_lookup.product_id', $listTap)
            ->pluck($this->_PRFIX_TABLE . '_wc_order_product_lookup.product_id');

        $result = $listTap->diff($listTapOrder)->values()->all();
